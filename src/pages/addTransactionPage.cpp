...
void AddTransactionPage::Init(float windowWidth, float windowHeight) 
{
  // Load logo
  ...
  GUI::TextStyle tooltipStyle = transactionTypeStyle;
  ...
  // Text component(s)
  GUI::Text header = GUI::Text(Rectangle{ 0, 0.2f*windowHeight, windowWidth, 0.1f*windowHeight }, headerStyle, "Create a Transaction");
  GUI::Text tooltip = GUI::Text(Rectangle{ 0 }, tooltipStyle, ""); // Information gets set when rendered

  // Button component(s)
  GUI::Button backButton = GUI::Button(Rectangle{ 0, 0, 0.1f*windowWidth, 0.1f*windowHeight }, backButtonStyle, "<- Back");
  GUI::Button createTransactionButton = GUI::Button(Rectangle{ 0.5f*windowWidth-(0.25f*windowWidth), 0.75f*windowHeight, 0.5f*windowWidth, 0.05f*windowHeight }, createTransactionButtonStyle, "Create Transaction!");
  ...
  // Load all the components into the GUI 'structure'
  textElements["header"] = header;
  textElements["tooltip"] = tooltip;
...
void AddTransactionPage::LoadData()
{
  std::vector<std::string> accountNames = { "Custom" };
  ...
  // Add the goals only for the 'to' section
...
int AddTransactionPage::Render(float windowWidth, float windowHeight, GUI::MouseState& mouseState)
{
  if (init)
  {
  ...
  {
    Rectangle bounds = inputElements["transactionFrom"].GetBounds();
    bounds.y += bounds.height+10.0f;
    bounds.width += 0.1f*windowWidth;
    
    textElements["tooltip"].SetBounds(bounds);
    textElements["tooltip"].SetText("*This is where the transaction will come from, either use the dropdown to select an account, or if you want something custom, i.e job, gift, etc... use the custom tag and fill out the form.");
    textElements["tooltip"].UpdateAndRender(mouseState);
  }
  {
    Rectangle bounds = inputElements["transactionTo"].GetBounds();
    bounds.y += bounds.height+10.0f;
    bounds.width += 0.1f*windowWidth;
    
    textElements["tooltip"].SetBounds(bounds);
    textElements["tooltip"].SetText("*This is where the transaction will go to, either use the dropdown to select an account or a goal, or if you want something custom, i.e job, gift, etc.. use the custom tag and fill out the form.");
    textElements["tooltip"].UpdateAndRender(mouseState);
  }
  ...
  if (buttonElements["createTransaction"].UpdateAndRender(mouseState) || IsKeyPressed(KEY_ENTER))
  {
    // Add transaction 
    ...
    if (transactionPeriod != "Not recurring")
    {
      uint8_t period = std::stoi(transactionPeriod.substr(0, transactionPeriod.find(' ')));
      Database::AddRecurring(from, to, period, transactionAmount, 0);
    }

    // Add the transaction no matter what, even if it is reccuring
    Database::AddTransaction(from, to, transactionAmount);
